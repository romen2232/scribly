 ...
 */
const createFolder = async (folder, token) => {
    try {
        const response = await apiClient.post('/api/v1/folders/', folder, {
            headers: {
 ...
 */
const retrieveFolder = async (id, token) => {
    try {
        const response = await apiClient.get(`/api/v1/folder/${id}/`, {
            headers: { Authorization: `Bearer ${token}` },
        });
        return response.data;
    }
 ...
 */
const updateFolder = async (id, folder, token) => {
    try {
        const response = await apiClient.put(`/api/v1/folder/${id}/`, folder, {
            headers: {
                Authorization: `Bearer ${token}`,
                'Content-Type': 'application/x-www-form-urlencoded',
            },
 ...
 * @throws Error if updating folder fails
 */
const partialUpdateFolder = async (id, folder, token) => {
    try {
        const response = await apiClient.patch(`/api/v1/folder/${id}/`, folder, {
            headers: {
                Authorization: `Bearer ${token}`,
                'Content-Type': 'application/x-www-form-urlencoded',
            },
 ...
 */
const destroyFolder = async (id, token) => {
    try {
...
const rootFolder = async (token) => {
    try {
        const response = await apiClient.get('/api/v1/user/folder/root', {
            headers: { Authorization: `Bearer ${token}` },
        });
        return response.data;
    }
...
export { listFolders, createFolder, retrieveFolder, updateFolder, partialUpdateFolder, destroyFolder, rootFolder, };
